...
from CPAN, neither a complete reference to some modules from
...
That said, what this book aims is to introduce a set of useful modules
and frameworks that are not easy to learn just by reading the main
Module documentation file (the one you usually get when executing
C<perldoc Module> on the command line).
...
chosen. Then, each section will focus in one of chose modules. And the
...
order: you can peek a chapter about a topic you are interested in, and
...
references. The usage of I<old> Object Oriented Perl might be useful
as well.
