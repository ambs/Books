...
CPANForum (L<http://cpanforum.com/>) offers a forum for each
...
CPANTesters (L<http://static.cpantesters.org/>) gives information
...
For the interactive shell, C<cpanp> also uses the C<i> command.
