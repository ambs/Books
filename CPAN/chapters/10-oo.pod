...
The definition of a role is simple. Just define a package and use
C<Role::Tiny>:
...
In your C<Moo> code, switch from role to role using C<with>. For
simplicity C<Moo> activates one role at a time:
...
limitations, like the lack of C<super> support. They are implemented
...
after some methods are called. To set some code to be run C<before> a
method call is as easy as:
...
method. This means that the code run should call the original
method. In this case, the function you define will receive as the
first argument the method that is being replaced.
