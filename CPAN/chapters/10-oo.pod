...
A few years ago C<Moose> appeared. It revolutionized the OO world of
Perl. During these years it has been enriched with a lot of relevant
and useful features.

Nevertheless, C<Moose> still imposes a delay upon initial compilation
and startup, which can be problematic for applications like CGI
scripts, which are parsed and run every time a request is made, for
which the overhead of loading C<Moose> may not be acceptable.

To minimize this problem some developers started C<Mouse>, a smaller
...
faster than C<Mouse>, although at the time of writing, it still has a relatively
small userbase in comparison.

Given that all these modules share the same basic syntax, this chapter
...
finally, what C<Moose> offers in addition.  It is hoped that this approach 
will make this chapter smaller and more helpful to the reader when choosing one
implementation.
...
There follows a simple example of a I<Person> object with two attributes,
I<name> and I<phone>. The first is a read only attribute (you can't
change the person's name after initializing the object) while the second
...
Note that the "fat comma" arrow  is equivalent to a comma, and the parenthesis
are not needed. The first attribute could be defined as:

  has "name", "is", "ro", "required", 1;

but its legibility is not the same. Therefore we will stick to the
usage of the fat comma arrow together with parenthesis or indentation to
group attribute options.

This object usage is similar to the usage of standard blessed
reference objects. C<Moo> creates the constructor and accessors automatically
for you.
...
type of data, you need to validate it yourself. That is done by adding a
code reference. The next example is a redefinition of the attribute
C<phone> requiring the value to consist of nine numeric digits:
...
If the default value needs to be computed based on other attributes, or
...
called over the object (receiving the object as the first parameter), with the 
new value as the second param. For instance, when a phone number is registered,
call some function to send a SMS to the new number acknowledging the change:
...
=begin Perl
 ...
 has phone =>
      ...
      trigger => sub {
         ...
         send_sms( to  => $new_number,
                   msg => "Hello there " . $self->name 
                       . ", phone number correctly updated, thanks!")
      };
...
One of C<Moo>'s sacrifices to remain small is to not support
...
C<Moo> activates only one role at a time. Our class now can call
C<dump> on a C<Person>:
...
The method modifiers described here are not really part of the object
...
itself. Return values are ignoredN<< In fact, you can can change C<@_>
directly, and the changed values will be received by the original
method. But this approach is not suggested. Use C<around> instead. >>.
...
to that  described above, to send a SMS message, but which uses the
...
scalar value (you can not use references).
...
You an redefine completely any attribute:

   has name => ( is => 'rw', isa => 'Str' );

Or you can use the superclass attribute definition as the base for
your new definition. If the line above this paragraph was in a
superclass, we can redefine the attribute as required with:

   has +name => ( required => 1 );

This would maintain the C<is> and C<isa> properties, and add the
C<required> flag.
...
Now, an object which represents stock exchange values can add stock
values together:
...
As a final note, note that unlike C<Moo>, C<Mouse> supports more than
one role at a time.
