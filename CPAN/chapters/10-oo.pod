...
types. For example, we know that the sex value will be one of I<male>
or I<female>:
...
C<superclasses> is an accessor that retrieves the list of all
super-classes for that class. It can also be used to set them.

   my @superclases = $meta->superclasses();

=item *

C<has\_method> returns a boolean value given a method name. The value
will be true if the class implements that method:

   $meta->has_method("compare");

=item *

C<add\_method> lets you add method to the class. It receives the method
name and the reference to a code block:
...
C<has\_attribute> works just like C<has\_method> but returns a true
value if there is an attribute with the supplied name:

   $meta->has_attribute("sex");
