...
Perl equivalent to what we are used to call classes). Follows a simple
example of a I<Person> object with two attributes, I<name> and
I<phone>. The first is a read only attribute (you can't change the
person name after initializing the object) while the second has
read/write permissions. This information is used by C<Moo> to define
whether to create accessors that are able to set and get values, or
just get values. The I<required> option means that every time a
I<Person> object is created a name must be supplied.
...
usage of the fat arrow together with parenthesis to group attribute
options.
