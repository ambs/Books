...
Nevertheless, C<Moose> still imposes a delay upon initial compilation and
startup, which can be problematic for applications like CGI scripts, which
are parsed and run every time a request is made, for which the overhead of
loading C<Moose> may not be acceptable.
