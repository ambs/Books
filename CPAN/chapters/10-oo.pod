...
Nevertheless, C<Moose> has a big load time (or compilation time, if
you prefer).  This means that this load time will be an overhead for
applications that should start and act quickly, like CGI scripts, that
are parsed and compiled every time a request is made, or a command
line tool, that should load and execute as fast as possible.
